package com.example.javatest.AopTest.j_proxy.factory;
...
public class BeanFactory {
    ...
    // 使用静态代码块初始化properties，加载factory_j.properties文件
    static {
    ...
    public static Object getBeanV2(String beanName) {
        // 双检锁保证beanMap中确实没有beanName对应的对象
        if (!beanMap.containsKey(beanName)) {
            synchronized (BeanFactory.class) {
                if (!beanMap.containsKey(beanName)) {
                    ...
                    try {
                        ...
                        if(proxyAdvisorClassName != null && proxyAdvisorClassName.length() > 0){
                            Class<?> proxyLogDivisor = Class.forName(proxyAdvisorClassName);

                            List<String> methods = Arrays.asList(properties.getProperty("demoService.proxy.methods").split(","));
                            InvocationHandler proxyHandler = (InvocationHandler) proxyLogDivisor.getConstructors()[0].newInstance(bean, methods);
